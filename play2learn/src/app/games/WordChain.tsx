...
export default function WordChain({
  ...
  useEffect(() => {
    ...
    socket.on("word", (data) => {
      const { word, playersWord } = data;
      setLastWord(word);
      setPlayers(playersWord);
      setCurrentWord("");
    });
            ...
            prevPlayers.map((player) =>
              player.isActive
                ? {
                    ...player,
                    word: currentWord,
                    localPoints: player.localPoints + 1,
                    time:0
                  }
                : player
            )
  ...
  const nextPlayer = () => {
    ...
    socket.emit("nextTurnGeneral", {
      roomUUID: game.uuid,
      user_id: localPlayer.user_id,
      points: localPlayer.localPoints,
    });
  };
  ...
  useEffect(() => {}, [players.find((p) => p.isActive), gameStarted]);
              ...
              >
                <img
                  ...
                  className={`w-16 h-16 rounded-full object-cover border-4 ${
                    player.isActive ? "border-white" : "border-transparent"
                  }`}
                />
